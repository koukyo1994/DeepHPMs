...
class DeepHPM:
    def __init__(self, t, x, u, x0, u0, tb, X_f, u_layers, pde_layers, layers,
    ...
    def idn_u_train(self, N_iter):
        ...
        for it in range(N_iter):
            ...
            if it % 10 == 0:
                ...
                print('It: %d, Loss: %.3e, Time: %.2f' % (it, loss_value,
                                                          elapsed))
                start_time = time.time()
    ...
    def sol_init(self, x0, u0, tb, X_f, layers):
        ...
        self.u_lb_pred, self.u_x_lb_pred = self.sol_net_u(
            self.t_lb_tf, self.x_lb_tf)
        self.u_ub_pred, self.u_x_ub_pred = self.sol_net_u(
    ...
    def sol_predict(self, t_star, x_star):
        ...
        f_star = self.sess.run(self.sol_f_pred, {
            self.t_f_tf: t_star,
            self.x_f_tf: x_star
        })
...
if __name__ == "__main__":
    ...
    # Model
    model = DeepHPM(t_train, x_train, u_train, x0_train, u0_train, tb_train,
                    X_f_train, u_layers, pde_layers, layers, lb_idn, ub_idn,
                    lb_sol, ub_sol)
    ...
    u_pred_identifier, f_pred_identifier = model.idn_predict(
        t_idn_star, x_idn_star)

    error_u_identifier = np.linalg.norm(u_idn_star - u_pred_identifier,
                                        2) / np.linalg.norm(u_idn_star, 2)
    ...
    error_u = np.linalg.norm(u_sol_star - u_pred, 2) / np.linalg.norm(
        u_sol_star, 2)
    print('Error u: %e' % (error_u))

    U_pred = griddata(
        X_sol_star, u_pred.flatten(), (T_sol, X_sol), method='cubic')
